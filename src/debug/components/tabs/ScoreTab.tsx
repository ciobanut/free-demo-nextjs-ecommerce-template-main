...
const ScoreTab: React.FC = () => {
    ...
    if (!data) {
        return (
            <div className="p-4 text-center text-gray-6 text-sm">
                Загрузка...
            </div>
        );
    }
    ...
    const statusConfig: Record<string, { text: string; color: string }> = {
        below_threshold:   { text: 'Скора пока не хватает для показа баннера', color: 'text-yellow-dark' },
        no_eligible_banner:{ text: 'Нет подходящего баннера для твоего профиля', color: 'text-gray-6' },
        already_shown:     { text: 'Баннер уже был показан в этой сессии', color: 'text-blue' },
    };
    const status = show_banner
        ? { text: 'Баннер показан ✓', color: 'text-green-dark' }
        : statusConfig[reason] ?? { text: reason, color: 'text-gray-6' };

    return (
        <div className="p-4 space-y-4">
            {/* Progress bar */}
            <div>
                <div className="flex justify-between text-xs text-gray-6 mb-1">
                    <span>Скор: <strong>{scoreInt}</strong></span>
                    <span>Порог: <strong>{thresholdInt}</strong></span>
                </div>
                <div className="w-full bg-gray-3 rounded-full h-3">
                    <div
            ...
            {positiveContributions.length > 0 && (
                <div>
                    <h4 className="text-xs font-semibold text-gray-7 mb-2">Уже засчитано</h4>
                    <div className="space-y-1">
                        {positiveContributions.map((c, i) => (
                            <div
                                key={`${c.reason}-${i}`}
                                className="flex items-center justify-between bg-green-light-6 border border-green-light-6 rounded px-3 py-2"
                            >
                                <div className="flex items-center gap-2">
                                    <span className="text-green-dark text-sm">✓</span>
                                    <span className="text-xs text-gray-7">{c.reason}</span>
                                </div>
            ...
            {negativeContributions.length > 0 && (
                <div>
                    <h4 className="text-xs font-semibold text-gray-7 mb-2">Штрафы</h4>
                    <div className="space-y-1">
                        {negativeContributions.map((c, i) => (
                            <div
                                key={`${c.reason}-${i}`}
                                className="flex items-center justify-between bg-red-light-6 border border-red-light-6 rounded px-3 py-2"
                            >
                                <div className="flex items-center gap-2">
            ...
            {inactiveSignals.length > 0 && (
                <div>
                    <h4 className="text-xs font-semibold text-gray-7 mb-2">Что ещё поможет</h4>
                    <div className="space-y-1">
                        {inactiveSignals.map(([key, meta]) => (
                            <div
                                key={key}
                                className="flex items-center gap-2 bg-gray-1 border border-gray-2 rounded px-3 py-2"
                            >
                                <span className="text-gray-5 text-sm flex-shrink-0">○</span>
                                <span className="text-xs text-gray-6">{meta.tip}</span>
                            </div>
                        ))}
                    </div>
                </div>
