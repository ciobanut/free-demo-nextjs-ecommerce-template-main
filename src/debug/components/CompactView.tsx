...
const CompactView: React.FC<CompactViewProps> = ({ onExpand }) => {
    ...
    return (
        <div className="fixed bottom-4 right-20 z-[9999]">
            <button
                onClick={onExpand}
                className="bg-dark hover:bg-blue-dark font-semibold text-white px-4 py-2 rounded-lg shadow-lg transition-colors duration-200 flex items-center gap-2"
            >
                <span>Debug</span>
                {(trackCount > 0 || predictCount > 0) && (
                    <span className="bg-red font-semibold text-white text-xs px-2 py-1 rounded-full">
