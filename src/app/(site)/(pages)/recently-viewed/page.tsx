      ...
      <section className="py-20 lg:py-25">
        <div className="max-w-[1170px] mx-auto px-4 sm:px-8 xl:px-0">
          {recentlyViewedItems.length === 0 ? (
            ...
            <>
              <h2 className="font-semibold text-2xl text-dark mb-8">
                Recently Viewed Products ({recentlyViewedItems.length})
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-7.5">
                {recentlyViewedItems.map((item) => (
                  <ProductItem key={item.id} item={item as unknown as Product} />
                ))}
              </div>
