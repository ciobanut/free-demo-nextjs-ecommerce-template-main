...
const ProductItem = ({ item }: { item: Product }) => {
  ...
  const isInWishlist = useAppSelector((state) =>
    state.wishlistReducer.items.some((i) => i.id === item.id)
  );
  ...
  return (
    <div className="group">
      <div className="relative overflow-hidden flex items-center justify-center rounded-lg bg-[#F6F7FB] min-h-[270px] mb-4">
        ...
        <div className="absolute left-0 bottom-0 w-full flex items-center justify-center gap-2.5 pb-5 ease-linear duration-200">
          <button
            onClick={() => {
              openModal();
          ...
          <button
            ...
            className="add_to_cart inline-flex font-medium text-custom-sm py-[7px] px-5 rounded-[5px] bg-blue text-white ease-out duration-200 hover:bg-blue-dark"
          >
            Add to cart
          </button>
          ...
          <button
            ...
            id="favOne"
            className={`flex items-center justify-center w-9 h-9 rounded-[5px] shadow-1 ease-out duration-200 ${isInWishlist ? "bg-red text-white" : "bg-white text-dark hover:text-blue"}`}
          >
